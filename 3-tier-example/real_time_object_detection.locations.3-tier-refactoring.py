...
# T. Harris 9-Jan-2021 night -- refactor to 3-tier model with functions

# TO DO -- FINISH REFACTORING -- at this moment, SOMETHING IS WRONG BECAUSE
#          RECTANGLES ARE NOW IN WRONG LOCATION ON SCREEN

# Command line

# Command line:  python real_time_object_detection.locations.3-tier-refactoring.py \
#    --prototxt MobileNetSSD_deploy.prototxt.txt \
#    --model MobileNetSSD_deploy.caffemodel \
#    --video <filename of mp4 video file>
# Or one one line for copy/paste:
# python real_time_object_detection.locaitons.3-tier-refactoring.py --prototxt MobileNetSSD_deploy.prototxt.txt --model MobileNetSSD_deploy.caffemodel --video <filename of mp4 video file>
...
import math # TH added for calculations such as square root

# Function Definitions

# Data tier functions

def detectDancersDataTier(frame):

    """This function returns matrix of dancers' coordinates,
    and how many detected. Also height and width of frame."""
    ...
    return scaleDetectionsNonempty, numDetections, h, w

# Application tier functions (also known as "business logic")


# Presentation tier functions

def defineColorConstantsPresentationTier():

    """This function defines color constants for text and box display."""
    
    # TH added color constants RGB
    BLACK = (   0,   0,   0)
    RED   = (   0,   0, 255)
    BLUE =  (   255, 0,   0)
...
for nFrame in range(nFrames):
    ...
    dancersCoordinates, numDancersDetected, h, w = detectDancersDataTier(frame)
    
    # TH find aggregate properties
    if numDancersDetected > 0:
            xMin = dancersCoordinates[:, [3, 5]].min().astype("int")
            yMin = dancersCoordinates[:, [4, 6]].min().astype("int")
            xMax = dancersCoordinates[:, [3, 5]].max().astype("int")
            yMax = dancersCoordinates[:, [4, 6]].max().astype("int")

            xCenters = dancersCoordinates[:, [3, 5]].mean(axis = 1).astype("int")
            yCenters = dancersCoordinates[:, [4, 6]].mean(axis = 1).astype("int")
    ...
    print("Identified {0} dancers".format(numDancersDetected))
    print("They form a rectangle with area {4} and these corners: ({0},{1}), ({2},{3})".format(xMin, yMin, xMax, yMax, area))

    # loop over the detections

    BLACK, RED, BLUE = defineColorConstantsPresentationTier()
    
    for nDetection in np.arange(0, numDancersDetected):
        # extract the confidence (i.e., probability) associated with
        # the prediction
        confidence = dancersCoordinates[nDetection, 2]
        ...
        if confidence > args["confidence"]:
            ...
            goodDetection = dancersCoordinates[nDetection]
            ...
            text = "{} % of frame {}: contains {} detections ".format(areaPercentOfFrame, nFrame, numDancersDetected)
